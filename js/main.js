...
copybtn.onclick = () => {
    ...
    navigator.clipboard.writeText(toCopy).then(() => {
        ...
        const funnyCopyMessages = copybtn.innerText = ["Copied!", "Double Copy!", "Triple Copy!", "Dominating!!", "Rampage!!", "Mega Copy!!", "Unstoppable!!", "Wicked Sick!!", "Monster Copy!!!", "GODLIKE!!!", "BEYOND GODLIKE!!!!", "KÄSEKUCHEN!!!", Array(16).fill(0).reduce(p => p + String.fromCharCode(Math.floor(Math.random() * 65535)), "")];
        ...
        copyTimeout = setTimeout(() => {
            ...
            copybtn.innerText = "Text im Discord-Format kopiert";
        }, 2000)

    }, (err) => {
        // We don't need to stop the users if they get a little too excited about the button
        if (copyCount > 2) return;
        alert("Das Kopieren ist aus irgendeinem Grund fehlgeschlagen. Lassen Sie uns versuchen, eine Warnung anzuzeigen. Vielleicht können Sie sie stattdessen kopieren.");
        alert(toCopy);
    });
}